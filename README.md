...
- `src/few_shot.py` - main script with few shot prompts generation, for now, it uses prepopulated examples, potential improvement: generate examples with LLM and use feature from **langchain** *example_selector* in *FewShotChatMessagePromptTemplate* that will select N closest examples to the query.
## How to run:
...
- run main script<br>
`python3 src/few_shot.py --openai_model='gpt-4-turbo' --temperature=0. --top_p=0.9` - you can play with different OpenAI version models and their parameters
